...
const makeRoutes = (pages: any[]) => {
	let routesArray:RouteRecordRaw[] = []
	pages.filter((path: any[]) => !path.some(childrenFilter))
  .map(async (path: any[]) => {
	...
	const eachRoute: RouteRecordRaw = {
		path: route,
		name,
		component,
		meta: {
...
const routes: RouteRecordRaw[] = makeRoutes(pages)  
